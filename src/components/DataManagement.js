...
const DataManagement = ({ user, onLogout }) => {
  ...
  // 移动端标签页状态
  const [mobileActiveTab, setMobileActiveTab] = useState('stats');
  ...
  const mobileTabs = [
    ...
    { id: 'actions', name: '操作', icon: Wrench }
  ];
  ...
  return (
    <div className="data-management">
      ...
      <div className="dm-content">
        ...
        {isMobile && (
          <div className="mobile-tabs">
            {mobileTabs.map(tab => (
              <button
                key={tab.id}
                className={`mobile-tab ${mobileActiveTab === tab.id ? 'active' : ''}`}
        ...
        {/* 上次清理时间 - 移动端置顶 */}
        {isMobile && stats.lastCleanup && (
          <div className="last-cleanup mobile-top">
            <Clock size={16} />
        ...
        {/* 统计信息 */}
        <div className={`stats-grid ${isMobile ? (mobileActiveTab === 'stats' ? 'mobile-show' : 'mobile-hide') : ''}`}>
        ...
        {/* 自动清理设置 */}
        <div className={`settings-section ${isMobile ? (mobileActiveTab === 'settings' ? 'mobile-show' : 'mobile-hide') : ''}`}>
        ...
        {/* 数据操作 */}
        <div className={`actions-section ${isMobile ? (mobileActiveTab === 'actions' ? 'mobile-show' : 'mobile-hide') : ''}`}>
          <div className="section-header">
            <Database size={20} />
            <h2>数据操作</h2>
          </div>
