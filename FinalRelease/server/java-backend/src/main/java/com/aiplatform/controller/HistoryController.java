...
public class HistoryController {
    ...
    public ResponseEntity<Map<String, Object>> batchOperation(
        ...
        try {
            ...
            List<Object> chatIdsObj = (List<Object>) request.get("chatIds");
            
            // 将 Object 列表转换为 Long 列表
            List<Long> chatIds = new ArrayList<>();
            if (chatIdsObj != null) {
                for (Object obj : chatIdsObj) {
                    if (obj instanceof Integer) {
                        chatIds.add(((Integer) obj).longValue());
                    } else if (obj instanceof Long) {
                        chatIds.add((Long) obj);
                    } else {
                        throw new BusinessException("无效的聊天ID类型");
                    }
                }
            }
